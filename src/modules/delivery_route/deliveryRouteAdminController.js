const deliveryRouteDao = require("./deliveryRouteDao");
...
const createDeliveryRoute = async (req, res) => {
    try {
        ...
        let checkUniqueData = await deliveryRouteDao.checkUniqueDeliveryRouteSlug(title);
        if (checkUniqueData) {
            return responseHelper.notFoundResponse(req, res, 'Route name already in use.');
        } 
        const resData = await deliveryRouteDao.create(requestData);
 ...
 * @returns
 */
const getAllDeliveryRoute = async (req, res) => {
    try {
        const { _page: originalPageValue, page, limit } = getPageAndLimit(req);
        const attributes = { exclude: ['createdAt', 'updatedAt', 'deletedAt'] };
        const addressList = await deliveryRouteDao.findAllWithAttributesAndPagination(attributes, page, limit)
        const resData = {
...
const getDeliveryRouteById = async (req, res) => {
    try {
        const { id } = req.params;
        const supplierData = await deliveryRouteDao.findById(id);
        if (!supplierData) {
...
const updateDeliveryRoute = async (req, res) => {
    try {
        ...
        delete updates.id;
        const routeData = await deliveryRouteDao.findById(id);
        if (!routeData) {
            return responseHelper.notFoundResponse(req, res, "Route Data not found!");
        }

        let checkUniqueData = await deliveryRouteDao.checkUniqueRouteAttributeUpdate('title', updates.title, routeData.id);
        if (checkUniqueData) {
            return responseHelper.notFoundResponse(req, res, 'Route name already in use.');
        }

        updates.updated_by = req.adminData.id;
        const [count, routeNewData] = await deliveryRouteDao.update(routeData.id, updates);
...
const deleteDeliveryRoute = async (req, res) => {
    try {
        ...
        const routeRes = await deliveryRouteDao.softDelete(routeData.id);
